/**
 * Careers page — category filter for opening cards.
 */
export function initCareersPage() {
    const filters = document.querySelectorAll('[data-career-filter]');
    ...
    const applyFilter = (category) => {
        cards.forEach((card) => {
            const matches = category === 'all' || card.dataset.careerCategory === category;
    ...
    filters.forEach((button) => {
        button.addEventListener('click', () => {
            ...
            applyFilter(button.dataset.careerFilter || 'all');
        });
    });
}
