/**
 * Innovation Lab page — category filter for initiative cards.
 */
export function initInnovationLabPage() {
    const filters = document.querySelectorAll('[data-innovation-filter]');
    const cards = document.querySelectorAll('[data-innovation-card]');
    ...
    const applyFilter = (category) => {
        cards.forEach((card) => {
            const matches = category === 'all' || card.dataset.innovationCategory === category;
    ...
    filters.forEach((button) => {
        button.addEventListener('click', () => {
            ...
            applyFilter(button.dataset.innovationFilter || 'all');
        });
    });
}
