/**
 * Community page — category filter for program cards.
 */
export function initCommunityPage() {
    ...
    const applyFilter = (category) => {
        cards.forEach((card) => {
            const matches = category === 'all' || card.dataset.communityCategory === category;
    ...
    filters.forEach((button) => {
        button.addEventListener('click', () => {
            ...
            applyFilter(button.dataset.communityFilter || 'all');
        });
    });
}
