/**
 * Partner Hub page — category filter for program cards.
 */
export function initPartnerHubPage() {
    const filters = document.querySelectorAll('[data-partner-filter]');
    ...
    const applyFilter = (category) => {
        cards.forEach((card) => {
            const matches = category === 'all' || card.dataset.partnerCategory === category;
    ...
    filters.forEach((button) => {
        button.addEventListener('click', () => {
            ...
            applyFilter(button.dataset.partnerFilter || 'all');
        });
    });
}
