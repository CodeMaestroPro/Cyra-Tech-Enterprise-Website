...
@section('title', $catalog['seo']['title'] ?? 'Innovation Lab')
...
@push('head')
    ...
    @endif
    <meta property="og:title" content="{{ $catalog['seo']['title'] ?? 'Innovation Lab' }}">
    ...
    <main id="main-content">
        <section class="cyra-page-hero">
            ...
            <div class="cyra-container relative cyra-section-hero-inner">
                ...
                @endif
                <h1 class="mt-3 cyra-display">{{ $hero['title'] ?? 'Innovation Lab' }}</h1>
        ...
        <section class="cyra-section" aria-labelledby="innovation-catalog-title">
            <div class="cyra-container">
                <x-ui.section-heading
                    eyebrow="Lab Programs"
                    title="Research, prototyping, and production engineering"
                    id="innovation-catalog-title"
                    class="cyra-section-heading"
                />

                <div class="mb-8 flex flex-wrap gap-2" role="tablist" aria-label="Filter lab programs by category">
                    @foreach ($categories as $category)
                        <button
                            ...
                            data-innovation-filter="{{ $category['slug'] }}"
                            role="tab"
                ...
                <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-3" data-innovation-grid>
                    @foreach ($initiatives as $initiative)
                        <x-innovation-lab.initiative-card :initiative="$initiative" />
                    @endforeach
                </div>
            </div>
        </section>

        <section class="cyra-section cyra-section-soft border-t border-cyra-border" aria-labelledby="innovation-methodology-title">
            <div class="cyra-container">
                <x-ui.section-heading
                    :eyebrow="$methodology['eyebrow'] ?? null"
                    :title="$methodology['title'] ?? ''"
                    :description="$methodology['description'] ?? null"
                    id="innovation-methodology-title"
                    class="cyra-section-heading"
                />

                <ul class="grid gap-4 md:grid-cols-2">
                    @foreach ($methodology['points'] ?? [] as $point)
