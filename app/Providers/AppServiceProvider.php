<?php

namespace App\Providers;

use App\Models\User;
use App\Policies\RolePolicy;
use App\Policies\UserPolicy;
use App\Repositories\AboutPageRepository;
use App\Repositories\HomepageSectionRepository;
use App\Repositories\CareerOpeningRepository;
use App\Repositories\AnalyticsEventRepository;
use App\Repositories\MediaAssetRepository;
use App\Repositories\ProjectRepository;
use App\Repositories\ProjectTaskRepository;
use App\Repositories\CrmLeadRepository;
use App\Repositories\CmsPageRepository;
use App\Repositories\ContactInquiryRepository;
use App\Repositories\PartnerProgramRepository;
use App\Repositories\ClientAccountRepository;
use App\Repositories\ClientEngagementRepository;
use App\Repositories\InsightArticleRepository;
use App\Repositories\CommunityProgramRepository;
use App\Repositories\InnovationInitiativeRepository;
use App\Repositories\IndustryVerticalRepository;
use App\Repositories\LeadershipProfileRepository;
use App\Repositories\NavigationItemRepository;
use App\Repositories\PortfolioProjectRepository;
use App\Repositories\ProductOfferingRepository;
use App\Repositories\SolutionOfferingRepository;
use App\Repositories\PermissionRepository;
use App\Repositories\PlatformModuleRepository;
use App\Repositories\RoleRepository;
use App\Repositories\UserRepository;
use App\Services\AboutService;
use App\Services\AuthService;
use App\Services\CareerService;
use App\Services\AnalyticsService;
use App\Services\MediaLibraryService;
use App\Services\ProjectManagementService;
use App\Services\CrmService;
use App\Services\CmsService;
use App\Services\ContactService;
use App\Services\PartnerHubService;
use App\Services\ClientPortalService;
use App\Services\CommunityService;
use App\Services\DesignSystemService;
use App\Services\HomepageService;
use App\Services\InsightService;
use App\Services\InnovationLabService;
use App\Services\IndustryService;
use App\Services\LeadershipService;
use App\Services\NavigationService;
use App\Services\PortfolioService;
use App\Services\ProductService;
use App\Services\SolutionService;
use App\Services\DashboardService;
use App\Services\PlatformService;
use App\Services\RoleService;
use Illuminate\Support\Facades\Gate;
use Illuminate\Support\ServiceProvider;

class AppServiceProvider extends ServiceProvider
{
    public function register(): void
    {
        $this->app->singleton(PlatformModuleRepository::class);
        $this->app->singleton(NavigationItemRepository::class);
        $this->app->singleton(HomepageSectionRepository::class);
        $this->app->singleton(AboutPageRepository::class);
        $this->app->singleton(LeadershipProfileRepository::class);
        $this->app->singleton(SolutionOfferingRepository::class);
        $this->app->singleton(ProductOfferingRepository::class);
        $this->app->singleton(IndustryVerticalRepository::class);
        $this->app->singleton(PortfolioProjectRepository::class);
        $this->app->singleton(InnovationInitiativeRepository::class);
        $this->app->singleton(CommunityProgramRepository::class);
        $this->app->singleton(InsightArticleRepository::class);
        $this->app->singleton(CareerOpeningRepository::class);
        $this->app->singleton(AnalyticsEventRepository::class);
        $this->app->singleton(MediaAssetRepository::class);
        $this->app->singleton(ProjectRepository::class);
        $this->app->singleton(ProjectTaskRepository::class);
        $this->app->singleton(CrmLeadRepository::class);
        $this->app->singleton(CmsPageRepository::class);
        $this->app->singleton(ContactInquiryRepository::class);
        $this->app->singleton(PartnerProgramRepository::class);
        $this->app->singleton(ClientAccountRepository::class);
        $this->app->singleton(ClientEngagementRepository::class);
        $this->app->singleton(PermissionRepository::class);
        $this->app->singleton(RoleRepository::class);
        $this->app->singleton(UserRepository::class);
        $this->app->singleton(DashboardService::class);
        $this->app->singleton(PlatformService::class);
        $this->app->singleton(NavigationService::class);
        $this->app->singleton(HomepageService::class);
        $this->app->singleton(AboutService::class);
        $this->app->singleton(LeadershipService::class);
        $this->app->singleton(SolutionService::class);
        $this->app->singleton(ProductService::class);
        $this->app->singleton(IndustryService::class);
        $this->app->singleton(PortfolioService::class);
        $this->app->singleton(InnovationLabService::class);
        $this->app->singleton(CommunityService::class);
        $this->app->singleton(InsightService::class);
        $this->app->singleton(CareerService::class);
        $this->app->singleton(AnalyticsService::class);
        $this->app->singleton(MediaLibraryService::class);
        $this->app->singleton(ProjectManagementService::class);
        $this->app->singleton(CrmService::class);
        $this->app->singleton(CmsService::class);
        $this->app->singleton(ContactService::class);
        $this->app->singleton(PartnerHubService::class);
        $this->app->singleton(ClientPortalService::class);
        $this->app->singleton(DesignSystemService::class);
        $this->app->singleton(AuthService::class);
        $this->app->singleton(RoleService::class);
    }

    public function boot(): void
    {
        Gate::policy(User::class, UserPolicy::class);
        Gate::define('manage-roles', [RolePolicy::class, 'manage']);
        Gate::define('view-roles', [RolePolicy::class, 'viewAny']);
    }
}
